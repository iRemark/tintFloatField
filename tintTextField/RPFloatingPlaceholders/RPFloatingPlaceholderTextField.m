...
- (void)setupViewDefaults
{
    ...
    self.floatingLabel.font = [UIFont boldSystemFontOfSize:14.f];
...
- (void)showFloatingLabelWithAnimation:(BOOL)isAnimated
{
    ...
    self.floatingLabel.backgroundColor = [UIColor whiteColor];
...
- (void)adjustFramesForNewPlaceholder
{
    ...
    CGFloat offset = ceil(self.floatingLabel.font.lineHeight/2.0);
    
    CGSize titleSize = [self.floatingLabel.text sizeWithFont:self.floatingLabel.font constrainedToSize:CGSizeMake(MAXFLOAT, offset)];
    
    self.originalFloatingLabelFrame = CGRectMake(self.originalTextFieldFrame.origin.x + 5.f, self.originalTextFieldFrame.origin.y,
                                                 ceil(titleSize.width)+10, self.floatingLabel.frame.size.height);
    self.floatingLabel.frame = self.originalFloatingLabelFrame;
    self.floatingLabel.textAlignment = NSTextAlignmentCenter;
